...
func UpdateNodeDrainTimeoutInMachineDeployment(ctx context.Context, input UpdateNodeDrainTimeoutInMachineDeploymentInput) {
	//TODO: delete below
	log.Logf("wait-machine-updated interval: %+v", input.WaitForMachinesToBeUpdated)
	...
	for _, md := range input.MachineDeployments {
		...
		log.Logf("Patching the new value of nodeDrainTimeout to Machine Deployment %s/%s", md.Namespace, md.Name)
		log.Logf("value of md.Spec.template.Spec = %s", md.Spec.Template.Spec.NodeDrainTimeout)
		...
		//TODO: add interval time for khoang thoi gian cho doi cap nhat machines
		// IN docker.yaml
