...
func (h *apiServerHandler) apisDiscovery(req *restful.Request, resp *restful.Response) {
	...
	if req.PathParameter("group") != "" {
		...
		if req.PathParameter("group") == "coordination.k8s.io" && req.PathParameter("version") == "v1" {
			if err := resp.WriteEntity(coordinationV1APIResourceList); err != nil {
				_ = resp.WriteErrorString(http.StatusInternalServerError, err.Error())
				return
			}
			return
...
func getAPIResourceList(req *restful.Request) *metav1.APIResourceList {
	if req.PathParameter("group") != "" {
		...
		if req.PathParameter("group") == "coordination.k8s.io" && req.PathParameter("version") == "v1" {
			return coordinationV1APIResourceList
		}
		return nil
	}
